...
public class OrderDaoImpl  extends BaseDao<Order> implements OrderDao {
	...
	public List<Order> findNewOrders(long sid) {
		...
		CriteriaQuery<Order> query = builder.createQuery(Order.class);
		Root<Order> order=query.from(Order.class);
		Predicate condition =builder.and(builder.equal(order.get("storeId"),sid),builder.or(builder.equal(order.get("status"),"NEW"),builder.equal(order.get("status"),"ACCEPT"),builder.equal(order.get("status"),"SENDING")));
	    List <Order>orders=this.getEntityManager().createQuery(query.where(condition)).getResultList();
		return orders;
	}
