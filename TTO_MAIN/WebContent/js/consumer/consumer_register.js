$(function() {
	...
	$('#sign-form').ajaxForm({
		...
		beforeSubmit : function() {
			...
			var isName = verifyName(username);
			var isPsw = verifypassword(password, password2);
	...
	/*
	 * $("#sign-form").submit(function(){ $('#errorMsg2').text(""); var
	 * username=$('#username').val(); var password=$('#password').val(); var
	 * password2=$('#password2').val(); var isName=verifyName(username); var
	 * isPsw=verifypassword(password,password2); if(isName && isPsw){ $.ajax({
	 * type : "post", url : "../account/register.do", cache : false, data : {
	 * loginname : username, password : password }, error : function(error) {
	 * alert("error"); } }).done(function(json) { if (json != "") {
	 * if(json.isSuccess==true){ window.location.href='../consumer/search.view';
	 * }else{ $("#errorMsg2").show(); $("#errorMsg2").text(json.data); } } });
	 * }else{ $("#errorMsg2").show(); $('#errorMsg2').text(errorMsg); return
	 * false; } });
	 */
});
